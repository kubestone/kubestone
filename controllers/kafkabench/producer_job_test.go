...
var _ = Describe("Producer Job", func() {
	Describe("created from CR", func() {
		...
		BeforeEach(func() {
			...
			for _, test := range cr.Spec.Tests {
				jobs = append(jobs, NewProducerJob(&cr, &test))
			}
		})
		...
		Context("with ClusterInfo specified", func() {
			It("should contain broker list", func() {
				Expect(jobs[0].Spec.Template.Spec.Containers[0].Args).To(
					ContainElement("bootstrap.servers=" + strings.Join(cr.Spec.KafkaClusterInfo.Brokers, ",")))
			})
		})

		Context("with ExtraProducerOpts", func() {
			It("should contain options", func() {
				Expect(jobs[0].Spec.Template.Spec.Containers[0].Args).To(
					ContainElement("buffer.memory=671088640"))

				Expect(jobs[1].Spec.Template.Spec.Containers[0].Args).To(
					ContainElement("buffer.memory=671088640"))
				Expect(jobs[1].Spec.Template.Spec.Containers[0].Args).To(
					ContainElement("acks=1"))
			})
		})
	})
})
