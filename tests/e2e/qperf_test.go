...
const (
	qperfSampleCR = samplesDir + "/perf_v1alpha1_qperf.yaml"
)

var _ = Describe("qperf end to end test", func() {
	Context("creation from samples", func() {
		It("should create qperf-sample cr", func() {
			_, _, err := run("kubectl create -n " + e2eNamespaceQperf + " -f " + qperfSampleCR)
			Expect(err).To(BeNil())
		})
	})
	...
	Context("created job", func() {
		It("Should finish in a pre-defined time", func() {
			...
			namespacedName := types.NamespacedName{
				Namespace: e2eNamespaceQperf,
				Name:      "qperf-sample",
			}
			Eventually(func() bool {
				if err := client.Get(ctx, namespacedName, cr); err != nil {
					Fail("Unable to get qperf CR")
				}
